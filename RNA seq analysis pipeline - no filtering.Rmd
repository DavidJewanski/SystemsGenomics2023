...
ggtitle("PCA of mice cerebellum samples") #+ theme_minimal() + xlab(str_glue("PC1, variance: ", round(variance_proportion[1], 3))
...
grouping_plot
ggsave("Grouping_DAVID_biological_rhythms.png",plot = grouping_plot, width = 3, height = 3, path = paste0(pwd2,"/plots"))
...
neurofilament_expr_YMP <- avg_sc_expr_YMP[Nef_gene_ids$ensembl_gene_id,] ### LOL. wtf, wwhat
...
layout(matrix(1:2, nrow = 1, byrow = T))
par(mar=c(3,3,3,3))
lineplot(neurofilament_expr_YMP, main = paste0(treatment, " (", nrow(neurofilament_expr_YMP),")")) # wie stelle ich das gut dar?
#          ylim = c(-6e-16, 6e-16))
