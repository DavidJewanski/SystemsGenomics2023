...
ggtitle("PCA: Mice cerebellum samples") #+ theme_minimal() + xlab(str_glue("PC1, variance: ", round(variance_proportion[1], 3))
...
```{r}
expr_aDR <- expr[,7:15]
treatment_aDR <- sort(unique(meta1_aDR$treatment))
...
cl_DEG_genes <- cutree(hcl_DEG_genes, k = 7) #k can be adjusted
heatmap.2(corr_DEG_genes, Rowv = as.dendrogram(hcl_DEG_genes), Colv = as.dendrogram(hcl_DEG_genes),
trace = "none", scale = "none", labRow = NA, labCol = NA, col = viridis,
...
par(mar=c(3,3,3,3))
boxplot(myelin_expr,
        main = paste0("myelination (", nrow(myelin_expr),")"))
