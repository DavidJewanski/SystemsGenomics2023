...
#corr_DEG <- cor(t(avg_expr_aDR[DEG_aDR,]), method = "spearman")
corr_DEG_mice <- cor(expr_aDR[DEG_aDR,], method = "spearman")
corr_DEG_genes <- cor(t(expr_aDR[DEG_aDR,]), method = "spearman")

#avg expr aDr seems to have wrong format. rows and columns in the wrong way? we want cor between genes but get corr between treatments groups
#corr_DEG <- cor(avg_expr_aDR[DEG_aDR,], method = "spearman")
...
cl_DEG <- cutree(hcl_DEG_genes, k = 10)
